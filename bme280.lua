...
-- Sourced from: https://github.com/wogum/esp12/blob/master/bme280.lua
--
-- Modified L. Doig 17 Jan 2016
-- Added in changable i2c address and shifted SDA/SCL pins to main config files
-- Therefore SDA_PIN, SCL_PIN & BME280_ADDR must be defined elsewhere.
--
-- Example:
-- t, p, h = dofile("bme280.lua").read()
-- Note the units are:
-- t, p, h [mC, Pa, m%]
-- eg: t/1000 = C, p/100 = hPa, h/1000 = %
------------------------------------------------------------------------------
local M
do
-- cache
...
local function r8(reg)
    local r = { }
    i2c.start(0)
    i2c.address(0, BME280_ADDR, i2c.TRANSMITTER)
    i2c.write(0, reg)
    i2c.stop(0)
    i2c.start(0)
    i2c.address(0, BME280_ADDR, i2c.RECEIVER)
    r = i2c.read(0, 1)
    i2c.stop(0)
    return r:byte(1)
    end
